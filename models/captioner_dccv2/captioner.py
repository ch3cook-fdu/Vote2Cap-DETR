...
from models.captioner_dccv2.generation_utils import generation
from models.captioner_dccv2.scst import SCST_Training
from models.captioner_dccv2.position_embedding import PositionEmbeddingCoordsSine
...
class captioner(nn.Module):
    ...
    def __init__(self, args, train_dataset):
        ...
        self.tokenB_proj = nn.Sequential(
        ...
        self.pos_emb3d = PositionEmbeddingCoordsSine(
            d_pos=self.embedding_size, pos_type='fourier', normalize=True
        )
    ...
    def prepare_object_representations(self, detector_output: dict, inputs: dict) -> dict:
        ...
        pos_embed = self.pos_emb3d(query_xyz, input_range=point_cloud_dims)
        pos_embed = self.pos_proj(pos_embed.permute(0, 2, 1))
        
        # ntoken x batch x nquery x channel -> batch x nquery x ntoken x channel
        query_feature = detector_output['prop_features'].permute(1, 2, 0, 3)
        prefix_feature = torch.cat(
            [
                self.tokenB_proj(query_feature[:, :, [1]]), 
                pos_embed.unsqueeze(2)
            ], dim=2
        )
        # batch x nprop x nprefix x channel, as RNN-like guidance
        detector_output['prefix_feature'] = prefix_feature
        ...
        context_feature = proposal_dimension_select(
            detector_output['enc_features'], k_near_indice.reshape(batch, -1)
        )   # batch x (nprop x k) x channel
        context_feature = context_feature.reshape(batch, nprop, -1, self.embedding_size)
        
        encoder_hidden_states = self.context_projector(context_feature)
        rel_pos_emb3d = self.transformer.transformer.wpe.weight[:encoder_hidden_states.shape[2]]
        rel_pos_emb3d = rel_pos_emb3d[None, None].repeat(batch, nprop, 1, 1)
        encoder_hidden_states += rel_pos_emb3d
        
        detector_output['encoder_hidden_states'] = encoder_hidden_states
    ...
    def forward(self, detector_output: dict, inputs: dict, is_eval: bool=False) -> dict:
        ...
        detector_output = self.prepare_object_representations(detector_output, inputs)
    ...
    def forward_training(self, detector_output: Dict, inputs: Dict) -> Dict:
        ...
        prefix_tokens = detector_output['prefix_feature']
