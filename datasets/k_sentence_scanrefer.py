...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.k = 1 if args.k_sentence_per_scene is None else args.k_sentence_per_scene
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train':
            scan_name = self.scanrefer[idx]['scene_id']
        ...
        if self.split == 'train':
            ...
            other_targets = list(set(self.gathered_language[scan_name].keys()) - {int(object_id)})
            chosen_one = np.random.choice(
                other_targets, size=self.k - 1, 
                replace=True if len(other_targets) < self.k - 1 else False
            )
            chosen_one = list(chosen_one) + [int(object_id)]
            
            scene_caption = []
            if scan_name in self.gathered_language:
                for instance_id in instance_bboxes[:, -1]:
                    if instance_id in chosen_one:
                        if instance_id == int(object_id):
                            caption = ' '.join(
                                self.scanrefer[idx]['token'] + [self.tokenizer.eos_token]
                            )
                        else:
            ...
            ret_dict['scan_idx'] = np.array(self.scan_names.index(scan_name)).astype(np.int64)
